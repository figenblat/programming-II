...
public class Animal implements GUIListener{
	...
	public boolean isMouseOver() {
		 ...
		 return (processing.mouseX > x - this.image.width / 2
			        && processing.mouseX < x + this.image.width / 2
			        && processing.mouseY > y - this.image.height / 2
			        && processing.mouseY < y + this.image.height / 2);
	}
	...
	*/
	public boolean isClose(int x, int y, int range) {
		return (x > this.x - image.width / 2 - range && x < this.x + image.height
				&& y > this.y - image.height / 2 - range && y < this.y + image.width);
	...
	*/
	public boolean isClose(Animal otherAnimal, int range) {
		...
		return (otherAnimal.x>this.x-image.width/2-range && otherAnimal.x<this.x +image.width/2+range 
				&& otherAnimal.y>this.y-image.width/2-range && otherAnimal.y<this.y +image.width/2+range);
